...
3. **Elevation** — USGS EPQS
   (`epqs.nationalmap.gov`), disk-cached per coordinate.
...
| Ride progress during navigation | Live GPS when on-route; simulated advancement at model speeds otherwise (demo affordance, clearly bounded in `ActiveRideModel`) |
| Voice guidance | Mute toggle is a placeholder; speech synthesis not implemented |
